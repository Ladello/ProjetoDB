...
function retornaIdPorNome(string $nome) {
    global $pdo;
    $stament = $pdo->prepare("SELECT id FROM usuario WHERE nome = ?");
    $stament->execute([$nome]);
    $usuario = $stament->fetch(PDO::FETCH_ASSOC);
    return $usuario ? $usuario['id'] : null;
}

function alterarUsuario(int $id, string $nome, string $email, string $nivel): bool {
    global $pdo;
